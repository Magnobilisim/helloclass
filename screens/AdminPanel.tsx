...
import { 
  ...
  Settings, BellRing, LayoutDashboard, Filter, Send, ChevronRight 
} from 'lucide-react';
...
const Modal: React.FC<{ title: string, onClose: () => void, children: React.ReactNode }> = ({ title, onClose, children }) => (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl p-6 animate-in zoom-in-95 duration-200 relative">
...
const AdminPanel: React.FC = () => {
  ...
  const handleAddQuestion = (e: React.FormEvent) => {
    ...
    setNewQText(''); setNewQTopic(''); setNewQSubTopic(''); 
    setNewQOptions(['', '', '', '']);
  };
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className="w-72 bg-white border-r border-slate-100 flex flex-col shrink-0 z-20 shadow-xl shadow-slate-200/50">
        <div className="p-8">
          ...
          <p className="text-xs font-bold text-slate-400 ml-1 uppercase tracking-widest opacity-70">Admin Panel</p>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50/50">
        ...
        <header className="bg-white/80 backdrop-blur-md border-b border-slate-100 px-8 py-4 flex justify-between items-center sticky top-0 z-10">
            <h2 className="text-xl font-black text-slate-800 capitalize tracking-tight">{activeTab === 'metadata' ? 'Okul & Konu Yönetimi' : activeTab}</h2>
            <div className="flex items-center gap-4">
                <button onClick={() => setIsBroadcastModalOpen(true)} className="flex items-center gap-2 bg-slate-900 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-slate-800 transition-colors shadow-lg shadow-slate-200">
                    <BellRing size={16} /> Duyuru Yap
                </button>
            </div>
        </header>
                        ...
                        reportedPosts.map(post => (
                            <div key={post.id} className="bg-white rounded-3xl p-6 shadow-sm border border-red-100 relative overflow-hidden group">
                                ...
                                <div className="flex justify-between items-start mb-4 pl-4">
                                    ...
                                    <div className="flex items-center gap-2 bg-red-50 text-red-600 px-3 py-1 rounded-full text-xs font-black uppercase tracking-wide">
            ...
            {activeTab === 'users' && (
                <div className="space-y-4 animate-in fade-in">
                    <div className="flex gap-4 mb-6">
                        ...
                        <button onClick={() => setIsUserModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 shadow-lg shadow-indigo-200 transition-transform active:scale-95">
            ...
            {activeTab === 'questions' && (
                <div className="space-y-4 animate-in fade-in">
                     <div className="flex gap-4 mb-6">
                        ...
                        <button onClick={() => setIsQuestionModalOpen(true)} className="bg-violet-600 hover:bg-violet-700 text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 shadow-lg shadow-violet-200 transition-transform active:scale-95">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in">
                     ...
                     <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm h-fit">
                         ...
                         <div className="flex gap-2 mb-4">
                             ...
                             <button onClick={() => { if(newSchoolName) { adminAddSchool(newSchoolName); setNewSchoolName(''); }}} className="bg-slate-900 text-white px-4 rounded-xl hover:bg-slate-800"><Plus size={20}/></button>
                         </div>
                     ...
                     <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm h-fit">
                         ...
                         <div className="flex gap-2 mb-4">
                             ...
                             <button onClick={() => { if(newTopicName) { adminAddExamTopic(newTopicName, newTopicCategory); setNewTopicName(''); }}} className="bg-slate-900 text-white px-4 rounded-xl hover:bg-slate-800"><Plus size={20}/></button>
                         </div>
                         <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                             {allExamTopics.map(t => (
