...
import { Play, TrendingUp, Clock, Bot, Trophy, ArrowRight } from 'lucide-react';
import { Link, useNavigate } from 'react-router-dom';

export const StudentDashboard = () => {
  const { user, exams, users, t, availableSubjects } = useStore();
          ...
          {availableExams.slice(0, 3).map(exam => {
            ...
            <div key={exam.id} className="bg-white p-5 rounded-3xl shadow-sm hover:shadow-md transition-shadow border border-gray-100 flex flex-col group">
              ...
              <div className="mt-auto flex items-center justify-between">
                 ...
                 <button onClick={() => startExam(exam.id)} className="bg-brand-500 text-white p-3 rounded-xl hover:bg-brand-600 transition-colors shadow-lg shadow-brand-200 group-hover:scale-105"><Play size={20} fill="currentColor" /></button>
              </div>
            </div>
          )})}
        </div>
