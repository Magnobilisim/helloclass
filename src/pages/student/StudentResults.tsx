                ...
                return (
                   <div key={res.id} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 flex flex-col hover:shadow-md transition-shadow relative overflow-hidden group">
                      ...
                      {!isDeleted && isOwnProfile && (
                          <div className="mt-4 pt-4 border-t border-gray-50 pl-4 flex justify-end"><Link to={`/student/exams`} className="text-xs font-bold text-gray-400 hover:text-brand-600 flex items-center gap-1 transition-colors">{t('view_in_market')} <ArrowRight size={12} /></Link></div>
                      )}
                   </div>
                );
             })}
